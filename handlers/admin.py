"""
handlers/admin.py — Admin panel for 『𝑮𝑷』 𝑮𝑯𝑶𝑺𝑻 𝑷𝑹𝑶𝑻𝑶𝑪𝑶𝑳 BOT.

Keyboard-first admin panel:
👥 USERS | 🚫 BANNED USERS | 📋 PENDING CASES | 🔎 SEARCH USER
🪙 ADD COINS | 💰 COINS FOR ALL | 🎁 REFERRAL SYSTEM | 📢 BROADCAST
👥 FORCE GROUP | 🎯 VERIFY TARGET | 📊 STATISTICS | ⚙️ BOT SETTINGS | 👑 MANAGE ADMINS

Access is strictly by Telegram user ID (never username). The OWNER is
protected from removal by normal admins.
"""
import logging
import os
import time
...
from keyboards.inline import (
    ...
    pagination_keyboard,
    coin_menu_keyboard,
    coin_all_confirm_keyboard,
    referral_menu_keyboard,
    force_group_menu_keyboard,
...
# ── Reply-keyboard router (ADMIN PANEL button) ────────────────────────────────
...
async def users_page_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # users_page:{page} or banned_page:{page}
    page = int(parts[1])
...
async def user_search_input(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user is None:
        await update.effective_message.reply_text(
            "❌ User not found. They must /start the bot first.", parse_mode="Markdown"
        )
        return
    await update.effective_message.reply_text(
        format_user_summary(user), parse_mode="Markdown",
        reply_markup=admin_panel_keyboard(),
    )
...
async def case_review_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Open a pending case for moderation review (moderation_keyboard)."""
    query = update.callback_query
    await query.answer()
    ...
    if history:
        hist_lines = "\n*Previous moderation history:*\n" + "\n".join(
            f"• {h.action} — {h.created_at.strftime('%Y-%m-%d')}"
            for h in history[:3]
        ) + "\n"
...
async def coin_menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if action == "history":
        ...
        if not txns:
            await query.edit_message_text(
                "📜 *No coin transactions yet.*", parse_mode="Markdown",
...
async def coin_amount_input(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int | None:
    ...
    if not text.isdigit():
        await update.effective_message.reply_text(
            "❌ Invalid amount. Send a number or /cancel.", parse_mode="Markdown"
        )
        return STATE_AWAIT_COIN_AMOUNT
    amount = int(text)
...
async def coin_all_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int | None:
    """💰 COINS FOR ALL — ask amount, then CONFIRM/CANCEL."""
    query = update.callback_query
    await query.answer()
    parts = query.data.split(":")
    # coin_all:amount:{amount} or coin_all:cancel
    if len(parts) >= 3 and parts[1] == "amount":
        ...
        await query.edit_message_text(
            ...
            f"Add *{amount} 🪙* to ALL eligible users?\n"
    ...
    if parts[1] == "cancel":
        ...
        await query.edit_message_text(
            "❌ Distribution cancelled.", parse_mode="Markdown",
            reply_markup=admin_panel_keyboard(),
        )
        return -1
    # First tap → ask amount
    await query.edit_message_text(
...
async def coin_all_confirm_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    context.user_data.pop("coin_all_amount", None)
    if not result.get("ok"):
        await query.edit_message_text(f"❌ {result.get('error')}", parse_mode="Markdown",
                                      reply_markup=admin_panel_keyboard())
        return
    await query.edit_message_text(
        f"✅ *Distribution complete!*\n\n"
        f"💰 {amount} 🪙 added to *{result['updated']}* users.\n"
...
async def referral_menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if action == "toggle":
        ...
        await query.edit_message_text(
            f"🔄 *Referral System {'ENABLED ✅' if new_state else 'DISABLED ❌'}*",
            parse_mode="Markdown", reply_markup=referral_menu_keyboard(),
        )
        return
...
async def referral_setting_input(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int | None:
    ...
    if not text.isdigit():
        await update.effective_message.reply_text(
            "❌ Invalid number. Try again or /cancel.", parse_mode="Markdown"
        )
    ...
    if flow == "reward":
        ...
        await update.effective_message.reply_text(
            f"✅ Referral reward set to *{value} 🪙*.", parse_mode="Markdown",
            reply_markup=referral_menu_keyboard(),
        )
    else:
        referral_service.set_min_usage(value)
        await update.effective_message.reply_text(
            f"✅ Minimum requirement set to *{value}*.", parse_mode="Markdown",
            reply_markup=referral_menu_keyboard(),
        )
...
async def force_group_menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if action == "toggle":
        ...
        await query.edit_message_text(
            f"🔄 *Force Join {'ENABLED ✅' if new_state else 'DISABLED ❌'}*",
            parse_mode="Markdown", reply_markup=force_group_menu_keyboard(),
        )
        return
    if action == "remove":
        ...
        await query.edit_message_text(
            "🗑 Force-join requirement removed.", parse_mode="Markdown",
    ...
    await query.edit_message_text(
        ...
        f"Status: {status}\n"
        f"Target: {target}\n"
        f"Title: {cfg.chat_title or '—' if cfg else '—'}",
        parse_mode="Markdown", reply_markup=force_group_menu_keyboard(),
    )
...
async def force_group_input(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int | None:
    ...
    if not text:
        await update.effective_message.reply_text(
            "❌ Invalid target. Send a username or ID.", parse_mode="Markdown"
        )
    ...
    try:
        ...
        pass
    force_group_service.configure(chat_id=chat_id, chat_username=target_username,
                                  chat_title=title, enabled=True,
                                  updated_by=update.effective_user.id)
    context.user_data.pop("force_group_flow", None)
    await update.effective_message.reply_text(
        f"✅ *Force group configured!*\n"
        f"Target: `{target_username or chat_id}`\n"
        f"Title: {title or '—'}\n"
        f"Status: ✅ Enabled",
        parse_mode="Markdown", reply_markup=force_group_menu_keyboard(),
    )
...
async def _show_stats(message) -> None:
    ...
    text = (
        ...
        f"🪙 Coins Distributed: {coins_distributed}\n"
        f"📋 Total Cases: {total_cases}\n"
        f"⏳ Pending Cases: {status_counts.get('PENDING', 0)}\n"
        f"🚫 Banned Cases: {status_counts.get('BANNED', 0)}\n"
        f"👑 Total Admins: {total_admins} ({active_admins} active)\n"
        "━━━━━━━━━━━━━━━━━━\n"
        f"🚫 Banned Users: {banned_users}\n"
        f"🧾 Evidence items: {ev_count}\n"
        f"📜 Audit entries: {audit_count}\n"
        f"🔑 Active sessions: {sessions}\n"
        f"⏱ Uptime: {format_uptime(time.monotonic() - _START_TIME)}\n"
...
async def add_admin_input(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not text.isdigit():
        await update.effective_message.reply_text(
            "❌ Invalid ID. Use /admin to retry.", parse_mode="Markdown"
        )
        return
    new_id = int(text)
    ...
    if existing:
        ...
        admin = repo.get_or_create(new_id, None)
...
async def admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if action == "users":
        ...
        await query.edit_message_text(
            f"🎁 *Referral System*\n\n"
            f"Reward: *{referral_service.reward} 🪙*\n"
            f"Min requirement: *{referral_service.min_usage}*\n"
            f"Status: *{'ON ✅' if referral_service.enabled else 'OFF ❌'}*",
            parse_mode="Markdown", reply_markup=referral_menu_keyboard(),
        )
        ...
        from handlers.attack import attack_command
