...
from .constants import (STATE_EDIT_CHOICE, STATE_AWAIT_EMAIL, STATE_AWAIT_VERIFICATION_TOKEN,
                        STATE_TARGET, STATE_EVIDENCE_TYPE, STATE_EVIDENCE_LINK, STATE_EVIDENCE_SCREENSHOT,
                        STATE_REASON, STATE_DESCRIPTION, STATE_REVIEW, STATE_AWAIT_BROADCAST,
                        STATE_AWAIT_EMAIL_CHANGE, STATE_AWAIT_COIN_USER, STATE_AWAIT_COIN_AMOUNT,
                        STATE_AWAIT_COIN_ALL_CONFIRM, STATE_AWAIT_FORCE_GROUP_ID,
                        STATE_AWAIT_REFERRAL_AMOUNT, STATE_AWAIT_REFERRAL_MIN)
...
def build_application() -> Application:
    ...
    from handlers.start import start_command, cancel_command, resend_command, email_input, verification_token_input
    auth_conv = ConversationHandler(
        entry_points=[CommandHandler("start", start_command)],
        states={
            STATE_AWAIT_EMAIL: [MessageHandler(filters.TEXT & ~filters.COMMAND, email_input)],
            STATE_AWAIT_VERIFICATION_TOKEN: [MessageHandler(filters.TEXT & ~filters.COMMAND, verification_token_input)],
        },
        fallbacks=[CommandHandler("cancel", cancel_command)],
        name="auth_conversation", persistent=False, per_message=False,
    )
    ...
    from handlers.ban_request import (ban_request_command, evidence_type_callback, reason_callback,
                                      review_callback, edit_callback, target_input, evidence_link_input,
                                      evidence_screenshot_input, description_input)
    ...
    def _ban_request_states():
        return {
            ...
            STATE_EVIDENCE_SCREENSHOT: [MessageHandler(filters.PHOTO, evidence_screenshot_input),
    ...
    app.add_handler(ConversationHandler(
        entry_points=[CommandHandler("ban_request", ban_request_command),
                      CommandHandler("report", ban_request_command),
                      CallbackQueryHandler(ban_request_command, pattern=r"^main:ban$")],
        states=_ban_request_states(),
        fallbacks=[CommandHandler("cancel", cancel_command)],
        name="ban_request_conversation", persistent=False, per_message=False,
    ))

    from handlers.cases import cases_command, case_action_callback, cases_page_callback, moderation_callback
    ...
    from handlers.admin import (admin_command, admins_command, stats_command, admin_callback,
                                admins_page_callback, users_command, banned_command, users_page_callback,
                                pending_cases_command, pending_page_callback, case_review_callback,
                                coin_menu_callback, coin_all_callback, coin_all_confirm_callback,
                                referral_menu_callback, force_group_menu_callback, manage_admins_callback,
                                admin_panel_button)
    ...
    from handlers.admin import (coin_user_input, coin_amount_input, coin_all_amount_input,
                                referral_setting_input, force_group_input, add_admin_input, remove_admin_input)
    ...
    settings_conv = ConversationHandler(
        ...
        states={STATE_AWAIT_EMAIL_CHANGE: [MessageHandler(filters.TEXT & ~filters.COMMAND, email_change_input)]},
        fallbacks=[CommandHandler("cancel", cancel_command)],
        name="settings_conversation", persistent=False, per_message=False,
    )
    ...
    broadcast_conv = ConversationHandler(
        ...
        states={STATE_AWAIT_BROADCAST: [MessageHandler(filters.TEXT & ~filters.COMMAND, broadcast_input)]},
        fallbacks=[CommandHandler("cancel", cancel_command)],
        name="broadcast_conversation", persistent=False, per_message=False,
    )
    app.add_handler(broadcast_conv)

    from handlers.keyboard_router import reply_keyboard_router, search_user_input_router
...
async def _help_command(update: Update, context) -> None:
    await update.effective_message.reply_text(
        ...
        "🚫 BAN REQUEST · 👤 MY PROFILE · 🪙 MY COINS · 🎁 REFERRAL · 📋 MY CASES · 👥 REQUIRED GROUP\n\n"
...
async def post_init(application: Application) -> None:
    ...
    logger.info("%s %s — bot ready. Admins: %s", config.BOT_NAME, config.BOT_ENTERPRISE, config.ADMIN_IDS)
